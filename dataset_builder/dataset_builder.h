...
static constexpr double fft_spectogram_min = -40;
static constexpr double fft_spectogram_max = 0;
...
public:
  ...
  DataBuilder(std::size_t sample_time_secs, uint32_t coupled_oscilators,
              uint32_t uncoupled_oscilators = 0, uint32_t starting_index = 0,
              uint32_t rand_seed = std::random_device{}())
      : rand_eng(rand_seed), num_samples(SAMPLE_RATE * sample_time_secs),
