...
static inline void WriteLine(const std::string &filename,
...
public:
  BMPWriterC(const std::array<std::array<uint32_t, W>, H> &a_data) {
    ...
    fileheader.file_size =
        fileheader.offset_data + img_data.size() * 4 + sizeof(colorheader);
  ...
  BMPWriterC(const std::array<std::array<double, W>, H> &a_data) {
    ...
    fileheader.offset_data =
        sizeof(fileheader.offset_data) + sizeof(infoheader);
    fileheader.file_size =
        fileheader.offset_data + img_data.size() * 4 + sizeof(colorheader);
...
private:
  ...
  uint32_t ValToColor(double val, ColorScaleType type=GREYSCALE) {
    const uint8_t val_u =
        std::clamp<uint8_t>(static_cast<uint8_t>(std::round(0xff * val)), 0, 0xff);
...
public:
  explicit MonoWaveWriterC(const std::vector<int16_t> &a_data) {
    ...
    std::memcpy(reinterpret_cast<char *>(wav_data.data()),
           reinterpret_cast<const char *>(a_data.data()), wav_data.size());
