...
void StringOscillatorC::PrimeString(double freq, double velocity) {
  const double amplitude_decay(MAX_AMPLITUDE_DECAY_RATE + (MIN_AMPLITUDE_DECAY_RATE - MAX_AMPLITUDE_DECAY_RATE) * amplitude_decay_factor);
  const double frequency_decay(MAX_FREQUENCY_DECAY_RATE + (MIN_FREQUENCY_DECAY_RATE - MAX_FREQUENCY_DECAY_RATE) * frequency_decay_factor);
  const double sustain_decay(MAX_FREQUENCY_DECAY_RATE + (MIN_FREQUENCY_DECAY_RATE - MAX_FREQUENCY_DECAY_RATE) * non_sustain_factor);
  const double amplitude_factor(MIN_AMPLITUDE_CUTOFF + (MAX_AMPLITUDE_CUTOFF- MIN_AMPLITUDE_CUTOFF) * start_amplitude_factor);
  const double max_freq_factor(base_frequency_coupled ? MAX_COUPLED_FREQUENCY_FACTOR : MAX_UNCOUPLED_FREQUENCY_FACTOR);
  const double frequency_factor(MIN_FREQUENCY_FACTOR + (max_freq_factor - MIN_FREQUENCY_FACTOR) * start_frequency_factor);
  const double amplitude_attack = (MIN_AMPLITUDE_ATTACK_RATE + (MAX_AMPLITUDE_ATTACK_RATE - MIN_AMPLITUDE_ATTACK_RATE) * amplitude_attack_factor) ;
...
double StringOscillatorC::NextSample(bool sustain) {
  ...
  if (frequency_state > MAX_UNCOUPLED_FREQUENCY_FACTOR ||
      amplitude_state < MIN_AMPLITUDE_CUTOFF) {
    sample_pos += SAMPLE_INCREMENT;
    return 0.0;
  }
  else {
    // generate sample.
...
std::string StringOscillatorC::ToJson() {
  std::string json_str = "{\n"
                         "\"start_phase\":" + std::to_string(phase_factor) + ",\n"
                         "\"start_frequency_factor\":" + std::to_string(start_frequency_factor) + ",\n"
                         "\"start_amplitude_factor\":" + std::to_string(start_amplitude_factor) + ",\n"
                         "\"sustain_factor\":" + std::to_string(non_sustain_factor) + ",\n"
                         "\"amp_decay_rate\":" + std::to_string(amplitude_decay_factor) + ",\n"
                         "\"amp_attack_delta\":" + std::to_string(amplitude_attack_factor) + ",\n"
                         "\"freq_decay_rate\":" + std::to_string(frequency_decay_factor) + ",\n"
                         "\"base_frequency_coupled\":" + std::to_string(base_frequency_coupled) + "\n"
                         "}";
  return json_str;
}
void StringOscillatorC::AmendGain(double factor) {
   start_amplitude_factor = std::clamp<double>(start_amplitude_factor*factor, 0.0, 1.0);
}

std::string StringOscillatorC::ToCsv(){
  std::string csv_str = "";
  csv_str +=  std::to_string(start_amplitude_factor) + "," +
...
std::unique_ptr<StringOscillatorC> StringOscillatorC::TuneString(uint8_t severity) {
  ...
  return std::make_unique<StringOscillatorC>(
      phase, start_frequency_factor, amplitude_factor, non_sustain_factor,
...
std::unique_ptr<StringOscillatorC> StringOscillatorC::CreateUntunedString(bool is_coupled) {
  ...
  return std::make_unique<StringOscillatorC>(
      phase, freq_factor, amplitude_factor, non_sustain_factor, amplitude_decay,
...
std::unique_ptr<StringOscillatorC> StringOscillatorC::CreateStringFromCsv( const std::string& csv_string) {
  ...
  double freq_factor = real_distr(stat_rand_eng);         // Maps to 0 to max_uncoupled_frequency_factor  or max max_coupled_frequency_factor
