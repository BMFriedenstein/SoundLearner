...
template <typename T, std::size_t A> static inline void WindowFunction(std::array<T, A> *in_out_data, WindowTypes window_function) {
  ...
  case WindowTypes::HANN: {
    ...
    std::transform(in_out_data->begin(), in_out_data->end(), in_out_data->begin(),
                   [&i, &scale_val](auto in_val) { return T(in_val * std::pow(std::sin(i++ / scale_val), 2)); });
  } break;
...
static inline std::array<std::array<To, Y>, X> CreateSpectrogram(const std::vector<Ti> &source_signal) {
  ...
  for (auto window = return_spectogram.begin(); window != return_spectogram.end(); ++window) {
    ...
    std::transform(frame.begin(), frame.end(), window->begin(),
                   [&](const auto &val) { return static_cast<To>(std::clamp<double>(val * slope_m, 0.0, max)); });
    source_idx += increment_size;
  }
  return return_spectogram;
}

template <std::size_t S> static inline std::array<std::array<uint32_t, S>, S> CreateMelSpectrogram(const std::vector<int16_t> &source_signal) {
  std::array<std::array<uint32_t, S>, S> mel_spectogram{{0}};

  constexpr std::size_t Q = S * 25;
  const double slope_m = 1.0 / (fft_spectogram_min - fft_spectogram_max);
  const double log_max = std::log2(static_cast<double>(4 * SAMPLE_RATE));
  const double log_increments = log_max / Q;

  auto freq_to_mel = [&log_increments, &log_max](float idx) -> double { return S * std::log2(1 + (idx * log_increments)) / log_max; };

  std::size_t source_idx = 0;
  const std::size_t increment_size = source_signal.size() / S;

  std::array<int16_t, Q> frame = {0};
  for (std::size_t x = 0; x < S; ++x) {
    std::memset(frame.data(), 0, sizeof(int16_t) * frame.size());
    std::memcpy(frame.data(), &source_signal[source_idx], sizeof(int16_t) * std::min(Q, source_signal.size() - source_idx));
    OneSidedFFT<int16_t, Q>(&frame, SAMPLE_RATE);

    std::array<std::vector<double>, S> log_bins{};
    for (std::size_t y = 0; y < frame.size(); ++y) {
      std::size_t mapped_y = static_cast<std::size_t>(std::round(freq_to_mel(y)));
      mapped_y = std::clamp(mapped_y, std::size_t(0), std::size_t(S - 1));
      log_bins[mapped_y].push_back(frame[y] * 1.0);
    }

    for (std::size_t y = 0; y < mel_spectogram[x].size(); ++y) {
      if (log_bins[y].empty()) {
        mel_spectogram[x][y] = 0;
      } else {
        const double average = 4 * static_cast<double>(std::accumulate(log_bins[y].begin(), log_bins[y].end(), 0)) / log_bins[y].size();
        mel_spectogram[x][y] = static_cast<uint32_t>(std::numeric_limits<uint32_t>::max() * std::clamp(slope_m * average, 0.0, 1.0));
      }
    }
    source_idx += increment_size;
  }
