 ...
 */
std::pair<std::vector<int16_t>, bool> InstrumentModel::GenerateIntSignal(double velocity, double frequency, std::size_t num_of_samples,
                                                                         bool return_on_distort) {
  bool has_distorted_out = false;
  std::for_each(sound_strings.begin(), sound_strings.end(), [&](const auto &s) { s->PrimeString(frequency, velocity); });
  ...
  for (std::size_t i = 0; i < num_of_samples; i++) {
    ...
    if (return_on_distort && has_distorted_out) {
      return std::make_pair(std::move(signal), std::move(has_distorted_out));
    }

    signal[i] = static_cast<int16_t>(std::numeric_limits<int16_t>::max() * sample_val);
  }
